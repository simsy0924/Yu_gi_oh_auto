...
function renderSetup(){
  root.innerHTML=portrait()+`<main class="screen setup"><header><div class="brand"><div class="brand-mark">GD</div><div><h1>Ghost Duel</h1><p>고스트와 연습하는 1인용 듀얼 · YGOPro Core</p></div></div><a href="https://github.com/simsy0924/Yu_gi_oh_auto" target="_blank" rel="noreferrer">소스 / 라이선스</a></header><section class="setup-grid"><div class="picker"><h2>고스트 선택</h2><div class="list" id="ghostList">${ghosts.map(x=>choiceItem(x,ghostId)).join('')}</div><label class="import">고스트 JSON 불러오기<input id="ghostFile" type="file" accept=".json"></label></div><div class="picker"><h2>내 덱 선택</h2><div class="list" id="deckList">${decks.map(x=>choiceItem(x,deckId)).join('')}</div><label class="import">YDK / JSON 덱 불러오기<input id="deckFile" type="file" accept=".ydk,.json"></label></div></section><footer class="setup-footer"><div class="selection"><span role="alert">${esc(error)}</span><p>기본 덱으로 바로 시작할 수 있어요. 금제 검사는 적용하지 않습니다.</p></div><button class="primary" id="start" ${!ghosts.length||!decks.length?'disabled':''}>DUEL START</button></footer></main>`;
  ...
  document.getElementById('deckFile').onchange=async e=>{try{const file=e.target.files[0];if(!file)return;const d=parseDeck(await file.text(),file.name);d.id=crypto.randomUUID();decks.push(d);deckId=d.id;error='';}catch(e){error=e.message;}renderSetup();};
  document.getElementById('ghostFile').onchange=async e=>{try{const file=e.target.files[0];if(!file)return;const g=JSON.parse(await file.text());g.deck=parseDeck(JSON.stringify(g.deck));if(!g.name||!['scripted','basic'].includes(g.behavior?.type))throw new Error('고스트 name, deck, behavior.type을 확인하세요.');g.id=crypto.randomUUID();ghosts.push(g);ghostId=g.id;error='';}catch(e){error=e.message;}renderSetup();};
...
try{const index=await json('./ghosts/index.json');ghosts=await Promise.all(index.map(e=>json(e.file)));ghostId=ghosts[0]?.id;const deck=await json('./decks/starter.json');deck.id='starter';decks=[deck];deckId=deck.id;renderSetup();}catch(e){error=e.message;renderSetup();}
