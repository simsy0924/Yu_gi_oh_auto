...
export async function openDeckBuilder(root,{source,onSave,onClose}) {
  ...
  root.innerHTML=`<main class="screen deck-builder"><header class="builder-header"><div><h1>덱 만들기</h1><small>카드를 골라 넣고 JSON으로 내보내세요</small></div><div class="builder-tools">${themeToggle()}<button class="action" id="builderClose">덱 선택으로</button></div></header><p id="catalogLoading" role="status">카드 목록을 불러오는 중...</p></main>`;
  ...
  root.innerHTML=`<main class="screen deck-builder"><header class="builder-header"><h1>덱 만들기</h1><label class="deck-name">덱 이름<input id="builderName" maxlength="80" value="${esc(draft.name)}"></label><div class="builder-tools"><button class="action" id="builderNew">새 덱</button><button class="action" id="builderClose">덱 선택으로</button>${themeToggle()}</div></header><div class="builder-layout"><section class="card-search" aria-label="카드 검색"><div class="search-controls"><input id="cardSearch" type="search" placeholder="카드명 · 카드 번호" aria-label="카드명 또는 번호"><select id="cardFilter" aria-label="카드 종류"><option value="">모든 카드</option>${kinds.map(([name])=>`<option value="${name}">${name}</option>`).join('')}</select><button class="mini clear-filters" id="clearFilters" type="button" aria-label="검색 조건 초기화">초기화</button></div><details class="search-filters-panel"><summary>상세 필터</summary><div class="search-filters"><label>효과 텍스트<input id="effectSearch" type="search" placeholder="효과에 적힌 단어"></label><label>종족<select id="raceFilter"><option value="">전체</option>${races.map(([bit,name])=>`<option value="${bit}">${name}</option>`).join('')}</select></label><label>속성<select id="attributeFilter"><option value="">전체</option>${attributes.map(([bit,name])=>`<option value="${bit}">${name}</option>`).join('')}</select></label><label>수치 종류<select id="statKindFilter"><option value="">전체</option><option value="level">레벨</option><option value="rank">랭크</option><option value="link">링크</option></select></label><label>수치<input id="statValueFilter" type="number" min="1" max="99" step="1" inputmode="numeric" placeholder="전체"></label></div></details><p class="builder-help">메인/엑스트라는 자동 분류돼요. 사이드 탭에서는 사이드에 추가돼요.</p><div id="searchResults" class="search-results"></div><div class="search-paging" id="searchPaging"></div></section><section class="deck-workspace" aria-label="편집 중인 덱"><nav class="deck-tabs" id="deckTabs"></nav><div id="deckContents" class="deck-contents"></div><details class="builder-detail" open><summary>카드 정보</summary><div id="builderDetail">이름을 눌러 카드 효과를 확인하세요.</div></details></section></div><footer class="builder-footer"><div><p id="builderStatus" role="status"></p><small id="draftStatus">작성 내용은 이 기기에 자동 저장됩니다. 금제 검사는 적용하지 않습니다.</small></div><button class="action" id="builderUse">저장하고 사용</button><button class="primary" id="builderExport">JSON 내보내기</button></footer></main>`;
