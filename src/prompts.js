...
export function makePrompt(m,cards) {
  ...
  const add=(label,response,kind='',card=null)=>p.choices.push({id:String(p.choices.length),label,response,kind,card});
  const command=(list,action,kind,verb)=>(list??[]).forEach((c,index)=>add(`${verb} · ${label(c)}`,{type,action,index},kind,c.code));
  switch(m.type) {
    case M.SELECT_IDLECMD:
      ...
      if(m.to_ep)add('턴 종료',{type,action:I.TO_EP,index:null},'end'); break;
    ...
    case M.SELECT_CHAIN:
      p.title='체인할 효과를 선택하세요'; m.selects.forEach((c,index)=>add(label(c),{type,index},'activate',c.code));
    ...
    case M.SELECT_CARD: case M.SELECT_TRIBUTE:
      ...
      p.selection={options:m.selects.map((c,index)=>({id:index,label:label(c),value:c.release_param??1})),min:m.min,max:m.max,tribute:m.type===M.SELECT_TRIBUTE};
    ...
    case M.SELECT_UNSELECT_CARD:
      p.title='소재 선택 / 선택 해제';[...m.select_cards,...m.unselect_cards].forEach((c,index)=>add(`${index<m.select_cards.length?'선택':'해제'} · ${label(c)}`,{type,index},'select',c.code));
      if(m.can_finish||m.can_cancel)add(m.can_finish?'선택 완료':'취소',{type,index:null},'finish');break;
...
// A deliberately small fallback policy. Card effects come from the core; this only chooses legal actions.
export function ghostChoice(p, behavior={}, cursor=0) {
  ...
  for(const kind of ['pass','summon','attack','battle','end','finish','no']) {
