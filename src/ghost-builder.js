...
export function openGhostBuilder(root,{source,decks,selectedDeck,onClose,onSave}) {
  ...
  root.innerHTML=`<main class="screen ghost-builder"><header class="builder-header"><div><h1>고스트 행동 만들기</h1><small>현재 패와 필드에서 가능한 행동을 우선순위대로 고릅니다</small></div><div class="builder-tools"><button class="action" id="ghostNew">새 고스트</button><button class="action" id="ghostClose">선택 화면으로</button>${themeToggle()}</div></header><div class="ghost-layout"><section class="ghost-settings"><h2>고스트 설정</h2><label>이름<input id="ghostName" maxlength="80" value="${esc(draft.name)}"></label><label>설명<input id="ghostDescription" maxlength="160" value="${esc(draft.description)}"></label><label>사용할 덱<select id="ghostDeck">${source?'<option value="source">현재 고스트의 덱</option>':''}${decks.map(d=>`<option value="${esc(d.id)}">${esc(d.name)} (${d.main.length}장)</option>`).join('')}</select></label><label>고정 시작 패 카드 번호 (쉼표로 구분)<input id="ghostStartingHand" inputmode="numeric" list="ghostCards" placeholder="비우면 무작위 시작 패" value="${esc(draft.startingHand?.join(', ')??'')}"></label><p class="builder-help">고스트 생성 모드에서 기록한 시작 패입니다. 이 번호를 바꾸거나 비우면 시작 상황을 바꿀 수 있습니다.</p><label>행동 방식<select id="ghostMode"><option value="priority">우선순위 · 매번 가능한 행동 선택</option><option value="sequence">순서대로 · 기존 JSON 방식</option></select></label><label>정해진 행동이 없을 때<select id="ghostFallback"><option value="basic">기본 행동으로 진행</option><option value="pause">고스트 중단</option></select></label><p class="builder-help">우선순위 방식은 매 선택마다 위에서부터 규칙을 확인해 가능한 첫 행동을 실행합니다. 패에 없는 카드 규칙은 건너뜁니다. 순서대로 방식은 기존 JSON의 단계를 차례대로 소비합니다.</p><p class="builder-help">카드 번호와 선택 번호는 필요한 단계에만 입력하세요. 선택 번호는 코어가 보여 주는 버튼의 0부터 시작하는 순서입니다.</p></section><section class="ghost-steps"><div class="ghost-steps-heading"><h2>행동 우선순위 <span id="ghostStepCount"></span></h2><button class="mini" id="ghostAdd">+ 행동 추가</button></div><div id="ghostStepList" class="ghost-step-list"></div></section></div><footer class="builder-footer"><div><p id="ghostStatus" role="status"></p><small id="ghostDraftStatus">작성 내용은 이 기기에 자동 저장됩니다. JSON을 내보내면 다른 기기에서 불러올 수 있습니다.</small></div><button class="action" id="ghostSave">저장하고 사용</button><button class="primary" id="ghostExport">JSON 내보내기</button></footer></main>`;
  ...
  $('ghostStartingHand').oninput=e=>{
    const value=e.target.value.trim();
    if(!value)delete draft.startingHand;
    else draft.startingHand=value.split(',').map(item=>item.trim()===''?NaN:Number(item.trim()));
    refresh();
  };
  ...
  function update(e){
    ...
    if(field==='indices'||field==='counters'){
      if(value.trim()){step[field]=value.split(',').map(s=>s.trim()===''?NaN:Number(s.trim()));if(field==='indices'){delete step.card;delete step.choice;}}else delete step[field];
    }else if(value==='')delete step[field];
    else {step[field]=['card','choice'].includes(field)?Number(value):value;if(field==='card'&&selectionRequests.has(step.on)){delete step.indices;delete step.choice;}if(field==='choice'&&selectionRequests.has(step.on)){delete step.indices;delete step.card;}}
    refresh();
  }
