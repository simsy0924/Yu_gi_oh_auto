...
export function openGhostBuilder(root,{source,decks,selectedDeck,onClose,onSave}) {
  ...
  function update(e){
    ...
    if(field==='indices'){
      if(value.trim()){step.indices=value.split(',').map(s=>s.trim()===''?NaN:Number(s.trim()));delete step.card;}else delete step.indices;
